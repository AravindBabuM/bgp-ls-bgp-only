...
operations and SHOULD be advertised when the router links have differing
capabilities.
...
(SR-TE), it is required for each router to advertise it's Prefix SID(s)
(refer {{RFC8402}}) that can be used to direct traffic via specific BGP
routers. Advertising such BGP Prefix SID for every BGP router provides
this key attribute via BGP-LS and avoids the requirement for the
consumer of the topology information (e.g. a controller or local TE
...
- For certain SR-TE uses, the Prefix Metric value MAY be included and
  it is set based on the SR-TE computation based on the link-state
  topology learnt via BGP-LS.
...
focuses on examples with MPLS dataplane instantiation for SR, the same
is applicable for the IPv6 dataplane instantiation (SRv6) as well.
...
controllers via {{RFC9857}}, which allows controllers to learn the state
of SR Policies instantiated on routers in the network. This document
completes the missing piece related to getting the BGP topology
information from all the routers to a controller as well as the local
SR-TE process on each router for their path computation requirements.
Beyond single-fabric deployments, Scale-Across scenarios that
interconnect multiple data centers via DCI or metro links introduce
additional TE requirements: asymmetric and irregular inter-domain
topology, scarce inter-DC link capacity, explicit latency constraints,
and shared risk link groups (SRLGs). The BGP-LS topology collection
specified in this document provides the complete visibility of nodes,
links, and their TE attributes needed for path computation in these
deployments.
...
for deployments that use PCEP in the BGP-only network.

The SR-TE-relevant topology collected via BGP-LS in a BGP-only fabric
includes:

- Node properties and BGP SR Prefix SIDs, which allow directing
  traffic to a specific BGP router. Traffic for BGP SR Prefix SIDs
  follows the path computed by the BGP decision process.

- Link properties and BGP Adjacency SIDs, which allow directing
  traffic over a specific BGP peer session or link.

- SR Policy state including end point, candidate path properties, and
  Binding SID, enabling traffic to be steered into SR Policies
  instantiated on the routers.

- TE metric, which can be used to tailor path selection for SR-TE
  objectives.

- TE administrative group (affinities) and SRLG attributes, which
  enable computation of paths that include or exclude specific links
  or that are mutually disjoint.

- Link delay and loss measurements, which enable computation of
  latency-constrained paths meeting SLA requirements.

- Node and Link Maximum SID Depth (MSD), which allows controllers to
  instantiate SR Policies within the forwarding capability of each
  router.

The BGP-LS topology is advertised to a centralized controller for
use-cases requiring centralized SR Policy computation, which can then be
signaled to the SR-TE head-end via PCEP or BGP-SRTE. Centralized
computation is particularly essential for Scale-Across deployments where
no single BGP router has visibility beyond its directly connected
neighbors; the full topology graph assembled by the centralized BGP-LS
speaker enables computation of disjoint paths, bandwidth-optimized
routes, latency-constrained paths, and SRLG-aware paths across domain
boundaries. The topology may also be distributed to any node in the BGP
fabric for use by its local SR-TE process for locally addressed
use-cases.
